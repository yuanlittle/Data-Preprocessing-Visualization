...
# 2.8 modifying settings in latticexyplot(price~carat)
xyplot(x*y*z~carat)
xyplot(price~carat | clarity)
...
# 7.7 latticist
# it appears latticist is no longer being updated and is not available for R 3.0 and beyond
...
#run these commands in the terminal if they do not work in the Rstudio environment

mtcars %>%
  ggvis(~wt, ~mpg) %>%
  layer_smooths(span = input_slider(0.5, 1, value = 1,label = "Smoothing span")) %>%
...
# bigvis no longer updated, will only work with old version of Rcpp, not on a fresh R install
# according to this tweet
# https://twitter.com/hadleywickham/status/562334236011528192
# ggstat and gggeom are being developed as production replacements
